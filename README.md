...
`@SpringBootTest` 통합 테스트(예: `S3FileStorageTest`)는 실제 인프라에 직접 연결한다.
실행 전 아래로 PostgreSQL + LocalStack 을 기동한다.

```bash
docker compose up -d
```

- LocalStack S3 엔드포인트: `http://localhost:4566`
- 버킷 `insurance-policies` 는 `docker/localstack/init-s3.sh` 가 자동 생성한다.